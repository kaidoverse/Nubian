...
const FilterSidebar = () => {
    ...
    const colors = ["red", "blue", "green", "black", "white", "beige", "brown", "yellow", "purple", "pink"];
    ...
    return (
        <div className='p-4'>
            ...
            <div className='mb-6'>
                ...
                {sizes.map((size) => (
                    <div key={size} className='flex items-center mb-1'>
                        <input type="checkbox" name='size' value={size} onChange={handleFilterChange} checked={filters.size.includes(size)} className='mr-2 h-4 w-4 text-blue-500 focus:ring-blue-300' />
                        <span className='text-gray-700'>
                            {size}
                        </span>
                    </div>
            ...
            <div className='mb-6'>
                ...
                {materials.map((material) => (
                    <div key={material} className='flex items-center mb-1'>
                        <input type="checkbox" name='material' value={material} onChange={handleFilterChange} checked={filters.material.includes(material)} className='mr-2 h-4 w-4 text-blue-500 focus:ring-blue-300' />
                        <span className='text-gray-700'>
                            {material}
                        </span>
                    </div>
                ...
                {brands.map((brand) => (
                    <div key={brand} className='flex items-center mb-1'>
                        <input type="checkbox" name='brand' value={brand} onChange={handleFilterChange} checked={filters.brand.includes(brand)}
                            className='mr-2 h-4 w-4 text-blue-500 focus:ring-blue-300' />
                        <span className='text-gray-700'>
                            {brand}
                        </span>
                    </div>
            ...
            <div className='mb-8'>
                <label className='block text-gray-600 font-medium mb-2'>
                    Price Range</label>
                <input type="range" name='price range' min={0} max={1000} value={priceRange[1]} onChange={handlePriceChange} className='w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer' />
                <div className='flex justify-between text-grey-600 mb-2'>
                    <span>GH₵0</span>
